 ...
 *
 * Un tableur ignore les blancs de tête avant d'examiner le premier caractère
 * significatif : une tabulation, un retour chariot ou des espaces avant `=`
 * (par exemple) sont donc un contournement documenté par l'OWASP. Le
 * caractère déclencheur est cherché après ces blancs, mais le préfixe est
 * appliqué à la chaîne d'origine, sans la rogner.
 */
const csvCell = (value) => {
  const texte = value === null || value === undefined ? '' : String(value);
  const sûr = /^\s*[=+\-@]/.test(texte) ? `'${texte}` : texte;
