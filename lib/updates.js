...
const configurerMisesAJour = ({
  ...
  /**
   * Vrai dès qu'une pose a été tentée.
   *
   * L'updater refuse par un `error` d'installer ce qu'il n'a pas téléchargé —
   * le même canal qu'une panne de réseau, que ce module passe volontairement
   * sous silence. Sans cette marque, un refus d'installation était donc avalé
   * comme un incident de réseau, et le bandeau restait sur « Fermeture du
   * Cahier… » pour toujours.
   *
   * Elle ne se relève jamais : si la pose a réussi, le processus s'en va ; si
   * elle a échoué, le Cahier tourne sans base et tout ce qui arrive ensuite se
   * rapporte encore à cette tentative.
   */
  let poseTentee = false;
  ...
  const installer = async () => {
    ...
    poseTentee = true;
    // `quitAndInstall` ne rend rien quand tout va bien — le processus s'en va.
    // L'attendre ne coûte donc que dans le cas qui nous intéresse : un refus a
    // le temps d'être signalé avant que la route ne réponde, et la page
    // l'apprend du même coup, sans attendre le battement suivant.
  ...
  updater.on('error', (erreur) => {
    ...
    // une pose refusée laisse le Cahier sans base — les services ont déjà été
    // arrêtés pour libérer le verrou. Se taire serait promettre une fermeture
    // qui n'arrivera pas, devant une application qui ne répond plus.
    if (poseTentee) {
      etat.poser({
        etape: ETAPES.ECHEC,
        message:
          'La nouvelle version ne s’est pas posée. Ferme puis rouvre le Cahier : elle s’installera en se fermant.',
      });
      return;
    }
